...
export default function LeafRain({ count = 100 }) {
  const containerRef = useRef(null);

  const leaves = useMemo(() => Array.from({ length: count }, (_, i) => i), [count]);
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      $leaves.forEach((leaf) => {
        ...
        const startXvw = gsap.utils.random(-10, 110, 1);          // start slightly off-screen
        const endXvw   = startXvw + gsap.utils.random(-10, 10, 0.1); // small horizontal drift
        const startYvh = gsap.utils.random(-20, -5, 1);           // start above the viewport
        const scale    = gsap.utils.random(0.5, 1.2, 0.01);
        const rot      = gsap.utils.random(-45, 45, 1);
        const duration = gsap.utils.random(8, 18, 0.1);           // fall speed
        ...
        gsap.to(leaf, {
          ...
          ease: "none",
          repeat: -1,
          delay: gsap.utils.random(0, 12, 0.1), // staggered starts
          onRepeat() {
    ...
    >
      ...
      {leaves.map((i) => (
        <svg
          ...
          style={{
            position: "absolute",
            color: "rgba(63, 174, 109, 0.7)", // tweak to your palette
